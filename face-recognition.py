...
# The CNN architecture used here is a variant of the inception architecture (https://arxiv.org/abs/1409.4842).
# More precisely, it is a variant of the NN4 architecture described in
# https://arxiv.org/abs/1503.03832 and identified as
# https://cmusatyalab.github.io/openface/models-and-accuracies/#model-definitions model in the OpenFace project.
# This notebook uses a Keras implementation of that model whose definition was taken from
# the Keras-OpenFace https://github.com/iwantooxxoox/Keras-OpenFace project.
# The architecture details aren't too important here, it's only useful to know that there is a
# fully connected layer with 128 hidden units followed by an L2 normalization layer on top of the convolutional base.
...
# Model training aims to learn an embedding f(x) of image x such that the squared L2 distance between
# all faces of the same identity is small and the distance between a pair of faces from different identities is large.
# This can be achieved with a *triplet loss* L that is minimized when the distance between an anchor image $x^a_i$ and a positive image $x^p_i$ (same identity) in embedding space is smaller than the distance between that anchor image and a negative image $x^n_i$ (different identity) by at least a margin $\alpha$.
...
# on real training data. I'll maybe provide a fully functional generator later.

# generator = triplet_generator()
# nn4_small2_train.compile(loss=None, optimizer='adam')
# nn4_small2_train.fit_generator(generator, epochs=1, steps_per_epoch=1)

# The above code snippet should merely demonstrate how to setup model training.
# But instead of actually training a model from scratch we will now use a pre-trained model
# as training from scratch is very expensive and requires huge datasets to achieve good generalization performance.
# For example, https://arxiv.org/abs/1503.03832 uses a dataset of 200M images consisting of about 8M identities.
# The OpenFace project provides pre-trained models at
# https://cmusatyalab.github.io/openface/models-and-accuracies/#pre-trained-models
# that were trained with the public face recognition datasets FaceScrub(http://vintage.winklerbros.net/facescrub.html)
# and CASIA-WebFace(http://arxiv.org/abs/1411.7923).
# The Keras-OpenFace project converted the weights of the pre-trained nn4.small2.v1 model to CSV files
...
# http://vis-www.cs.umass.edu/lfw/ dataset is used.
...
# the face images from the custom dataset must be aligned too.
# Here, we use Dlib for face detection and OpenCV for image transformation and cropping
...
# As described in the OpenFace pre-trained models
# https://cmusatyalab.github.io/openface/models-and-accuracies/#pre-trained-models) section,
# landmark indices 'OUTER_EYES_AND_NOSE' are required for model nn4.small2.v1.
# Let's implement face detection, transformation and cropping as 'align_image' function for later reuse.
...
# But we still do not know what distance threshold is the best boundary for making a decision between same identity
# and different identity.
# ### Distance threshold
# To find the optimal value for tau, the face verification performance must be evaluated on a range of distance
# threshold values. At a given threshold, all possible embedding vector pairs are classified as either
# *same identity* or *different identity* and compared to the ground truth.
# Since we're dealing with skewed classes (much more negative pairs than positive pairs), we use the
# F1 score(https://en.wikipedia.org/wiki/F1_score) as evaluation metric instead of
...
# but since nn4.small2.v1 is a relatively small model it is still less than what can be achieved by
# state-of-the-art models (> 99%). The following two histograms show the distance distributions of
# positive and negative pairs and the location of the decision boundary.
# There is a clear separation of these distributions which explains the discriminative
# performance of the network. One can also spot some strong outliers in the positive pairs class
...
# ### Face recognition
# Given an estimate of the distance threshold tau, face recognition is now as simple as calculating
# the distances between an input embedding vector and all embedding vectors in a database.
# The input is assigned the label (i.e. identity) of the database entry with the smallest
# distance if it is less than tau or label *unknown* otherwise.
# This procedure can also scale to large databases as it can be easily parallelized.
# It also supports one-shot learning, as adding only a single entry of a new identity might be sufficient
# to recognize new examples of that identity. A more robust approach is to label the input using the
# top $k$ scoring entries in the database which is essentially KNN classification with a Euclidean distance metric.
...
# Seems reasonable :-) Classification results should actually be checked whether (a subset of)
# the database entries of the predicted identity have a distance less than tau,
# otherwise one should assign an *unknown* label. This step is skipped here but can be easily added.
# Dataset visualization
# To embed the dataset into 2D space for displaying identity clusters,
